...
arm.id<-2 #arm indicator
###END FUNCTION INPUTS
#Read in survival times read by digizeit
digizeit<- read.csv("data_raw/NAC2.csv", header = FALSE)
t.S<-digizeit[,1]
S<-digizeit[,2]
...
#Output IPD
IPD_02<-matrix(c(t.IPD,event.IPD,arm),ncol=3,byrow=F)

IPD_02

save(IPD_02, file = "data/NAC2.RData")
